...
onComplete | function | callback when timer has finished. returns an array of three values in the following order:
restart: boolean specified whether the timer can start over
minutes: string|number specifies the number of minutes to apply for when the timer restarts
seconds: string|number specifies the number of seconds to apply for when the timer restarts
 | No |
 onTick | function(timePassed: number) | callback whenever the timer changes between two frames. it accepts the total time passed in the number of seconds.
  | No |
